...
## Applications
This was intended to be used as a database, and can be used as a server too, although with some limitations. This can specifically be used for a message forum, which has [already been implemented](https://skparab1.github.io/server/frontend.html). Further uses can include short url generator, as well as to make user accounts. I plan to implement a form data collector, like google forms, soon. Another idea which i plan to implement soon is some sort of email shenanigan that can privately send and receive messages (with the same github database idea) with accounts, although, this would have to be a private repo ig.

## Pros
Although not ideal, this algorithm has several aspects that make it interesting an unique. First of all, it is 100% github based, from data storage and data processing to website hosting, which makes everything on one platorm. This makes it really easy to manage, and does everything for free. You can also see the backend live, from issues to actions, which makes it easy to track data processing and is another "cool" feature of this concept. This is also, while slow, not the slowest, and can process write requests in less than a minute. In addition, this algorithm is made to overcome concurrency conflict issues, which makes it able to proccess as many as 2.5 write requests per second. This makes it a viable algorithm for a database application in which writing time is not important. However, it does have some limitations.
...
- Note: if you do want to use this for an app hosted on Github, then you can't make the Repo public, because Github will revoke the token (Because well duh you shouldn't be sharing your token, and i learned it the hard way). To bypass this, i made a seperate private repo which contains the token to actually open the issue and send the message data to github.
````
(async () => {
